---
name: pr-comments
description: Pull the active comments for the PR associated with the current branch, assign stable numbering, persist checklist state, and run discussion-first triage. Use when the user wants to inspect PR comments, review threads, unresolved feedback, or maintain a rolling PR comment checklist.
---

# PR Comments

Build and maintain a stable, discussion-first checklist of active PR comments.
...
7. Assign stable hierarchical numbers and persist state in `_scratch/_pr_reviews/pr-<number>.json`.
...
- Path: `_scratch/_pr_reviews/pr-<number>.json` where `<number>` is the PR number.
...
- Keep existing numbers from `_scratch/_pr_reviews/pr-<number>.json` when ids match.
...
- Never mention tool or vendor names in user-facing output.
- Never sign responses with any product or assistant name.
...
When a PR comment contains a reusable lesson, store it in `_scratch/_agent_notes/` under a topic-oriented markdown filename. Update an existing topic file instead of creating duplicates when appropriate.
