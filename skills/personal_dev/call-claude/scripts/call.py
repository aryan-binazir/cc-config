...
def resolve(skill_dir: Path) -> tuple[dict[str, Any], Path]:
    local_path = skill_dir / "call-claude.local.yaml"
    config = merge(
        load_yaml(skill_dir / "call-claude.example.yaml"),
        load_yaml(local_path),
    )
    unknown = sorted(set(config) - {"model", "effort", "timeout_ms"})
    if unknown:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Call Claude using merged example and local configuration."
    )
    ...
    try:
        ...
        print("claude is not available", file=sys.stderr)
        return 127
