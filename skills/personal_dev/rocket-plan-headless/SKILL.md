...
These exact commands take precedence over the generic
`call-cursor`/`call-codex`/`call-claude` skill conventions (for example,
`call-cursor` defaults to `-f`; critics must not use force mode).
...
when applicable. Missing auth, missing configured runners, unreachable
`origin/main`, inaccessible tickets, and unsafe dirty target worktrees are hard
stops.
