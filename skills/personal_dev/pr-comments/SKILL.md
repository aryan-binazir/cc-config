...
4. Drop resolved review threads and minimized, deleted, outdated, or inactive comments from output, while keeping prior state entries. If activity state is unavailable, treat returned items as active.
...
6. Merge with `_scratch/_pr_reviews/pr-<number>.json`, preserving existing ids and numbering.
7. Render the full active list every run.

## State

Store JSON at `_scratch/_pr_reviews/pr-<number>.json` (create `_scratch/_pr_reviews/` if missing):
...
This file is the single shared PR state store: the `pr-address-comments` skill writes its handled state into an `agent` object on the same `itemsById` entries. Preserve every `agent` field when updating items.
...
Discussion-first is mandatory: code changes start only after a decision on the relevant item.
