...
Shared pipeline for `rocket-plan`, `rocket-plan-sub`, and `rocket-plan-headless`.
Read this file fully once at invocation, before acting. In this file, `<skill>`
...
Use `<skill> <profile>` when provided; otherwise use `defaults.plan_profile`.
...
sub-agent transcript. If no valid JSON is returned, retry once with one fresh
preflight sub-agent. If the retry also returns no valid JSON, stop with
`delegated_preflight_invalid_output`.
...
and ticket/source access when applicable. Missing auth, missing configured
...
happen at any phase. The required preflight branch/worktree setup is the only
branch setup allowed before critique; do not create or switch any other
branches. If the critic times out after the configured budget, report the
...
validation/commit checkpoints, any variant-specific items required by the
invoking skill, and `$rocket-review <review-profile>` as the final step.
...
Applies only to capsule-based variants. Write a compact implementation capsule
next to the contract at `_scratch/_contracts/<branch>.implementation.md` before
code changes. This capsule, not the planning transcript, is the source passed to
the implementer.
...
These rules apply regardless of who implements (inline, sub-agent, or headless
runner); each variant's `SKILL.md` defines the execution mechanics.

- Implementation follows strict test-first cycles. Use the `tdd` skill if
  available; otherwise read `Implementation Discipline` from the details
  reference and follow that loop.
- Keep changes scoped to the contract and repo-local rules.
- Commit logical checkpoints and run targeted validation, then repo-required
  broader validation.
- Before each commit, run the lint command defined by repo rules when one
  exists; otherwise run `make lint` if that target exists; otherwise skip
  linting and say so in the checkpoint summary.
- Fix ordinary code/test failures silently. Stop only when a failure reveals
  real spec ambiguity or a required permission/tooling blocker.
