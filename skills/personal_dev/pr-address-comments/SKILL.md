...
4. Push the commit, then `reply <n> --commit <hash> --agent <your name>` (Claude, Codex, Cursor…; config `agent` is the fallback) per handled comment. Always reply directly to the comment with the commit hash, alone or in a sentence. The script targets the top-level thread comment for review comments (linking the instruction when it was a reply) and posts a PR comment linking back for issue comments and review summaries; it records commit, reply URL, and handled status. Failures exit 1 with `{"ok": false, "error", "hint"}` — relay both to the user.
