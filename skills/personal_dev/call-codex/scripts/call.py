...
def resolve(skill_dir: Path) -> tuple[dict[str, Any], Path]:
    local_path = skill_dir / "call-codex.local.yaml"
    config = merge(
        load_yaml(skill_dir / "call-codex.example.yaml"),
        load_yaml(local_path),
    )
    unknown = sorted(set(config) - {"model", "reasoning_effort", "timeout_ms"})
    if unknown:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Call Codex using merged example and local configuration."
    )
    ...
    try:
        ...
        print("Codex call timed out", file=sys.stderr)
        return 124
    except FileNotFoundError:
        print("codex is not available", file=sys.stderr)
        return 127
