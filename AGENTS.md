...
## Git conventions
- Branches for Jira-backed work should be named `amb/XXXX-XXX`, where `XXXX-XXX` is the Jira issue key.
- Commit message format: `type(PROJECT): description` (e.g. `fix(BBA): Added logging package`). Use only the Jira project key, not the full ticket key. Types: `feat`, `fix`, `chore`, `refactor`, `test`, `docs`, `build`, `ci`, `revert`. If there is no ticket, use `type(no-ticket): description` — never invent a plausible-looking ticket key.
