...
2. `_scratch/initial_review-{branch}.md` where `{branch}` comes from `git branch --show-current` (with `/` replaced by `-`)
...
2. Write the report to `_scratch/initial_review-{branch}.md`:
   - Get branch: `git branch --show-current`
   - Replace `/` with `-` in branch name
   - Create `_scratch/` directory if it doesn't exist
   - Write to `_scratch/initial_review-{branch}.md`
3. Confirm: "Report saved to `_scratch/initial_review-{branch}.md`"
