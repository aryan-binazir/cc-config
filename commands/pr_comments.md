...
When a PR comment teaches something worth remembering — a pattern, a gotcha, a domain rule, a better approach — capture it in `_scratch/_agent_notes/` as a markdown topic file (e.g., `_scratch/_agent_notes/error-handling.md`, `_scratch/_agent_notes/api-pagination.md`). These files are for agent consumption, not human readability.

- Before creating a new file, search `_scratch/_agent_notes/` for an existing file on the same topic. Update it if one exists.
